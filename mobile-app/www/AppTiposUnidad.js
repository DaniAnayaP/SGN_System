...
async function patchRecord(id, patch) {
    const recordKey = `tipos-unidad:${id}`;
    const before = currentRecord() || {};
    const description = `${t('menu.catTransVolTiposUnidades')} · ${recordLabel(before)}`;
...
async function deleteRecord(id) {
    try {
        const res = await fetch(apiUrl(`/api/business/unit-types/${id}`), { method: 'DELETE', credentials: 'include' });
...
function renderListView() {
    ...
    records.forEach((record) => {
        ...
        const offlineBadge = offlinePendingRecordIds.has(`tipos-unidad:${record.id}`)
