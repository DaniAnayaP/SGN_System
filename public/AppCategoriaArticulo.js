// ---------------------------------------------------------------------------
// Nuestras Categorías <Tipo> (App) — same one-field-at-a-time shell as Tipos
// de Unidad (see AppTiposUnidad.js for the original of this pattern): a
// catalog entry never "graduates out" the way a finished operational record
// does, so there's just one flat list (nothing ever disappears), tap any row
// to edit. One shared script instead of 7 copy-pasted ones -- see
// CatCentroDistCategoriaArticulo.js (the Web/table version) for why: these 7
// screens are the exact same shape by design, each HTML shell only differs
// in which category it points at via <body data-category-type="...">.
// ---------------------------------------------------------------------------
...
// --- Which of the 7 catalogs this page is -----------------------------------
const CATEGORY_TYPE_LABEL_KEYS = {
    inventario: 'menu.catCentroDistCategoriasInventario',
    compra: 'menu.catCentroDistCategoriasCompra',
    almacenamiento: 'menu.catCentroDistCategoriasAlmacenamiento',
    rotacion: 'menu.catCentroDistCategoriasRotacion',
    manejo: 'menu.catCentroDistCategoriasManejo',
    riesgo: 'menu.catCentroDistCategoriasRiesgo',
    vidautil: 'menu.catCentroDistCategoriasVidautil',
};
const CATEGORY_TYPE = document.body.dataset.categoryType;
const SCREEN_LABEL_KEY = CATEGORY_TYPE_LABEL_KEYS[CATEGORY_TYPE];
const API_BASE = `/api/business/article-categories/${CATEGORY_TYPE}`;

// --- Field catalog -----------------------------------------------------
const FIELDS = [
    { id: 'name', apiKey: 'name', labelKey: 'main.colCatName', hintKey: 'home.catHintName', type: 'text', icon: 'bx-purchase-tag' },
    { id: 'description', apiKey: 'description', labelKey: 'main.colCatDescription', hintKey: 'home.catHintDescription', type: 'text', icon: 'bx-note' },
    {
        id: 'status', apiKey: 'status', labelKey: 'main.colCatStatus', hintKey: 'home.catHintStatus', type: 'select', icon: 'bx-toggle-left',
...
// confirmed (see commitFieldValue) — "+ Nueva Categoría" just opens this
// in-memory draft (no id/code yet), so tapping it and backing out without
// typing anything never leaves a blank category behind.
let draftRecord = null;
function blankDraftRecord() {
    return { id: null, code: '', name: '', description: '', status: '', pendingFields: [] };
}
...
function recordLabel(record) {
    ...
    if (!record.id) return t('home.catNewButton');
    return `${t('home.catFallbackLabel')} ${record.code}`;
}
...
async function patchRecord(id, patch) {
    const recordKey = `${CATEGORY_TYPE}:${id}`;
    const before = currentRecord() || {};
    const description = `${t(SCREEN_LABEL_KEY)} · ${recordLabel(before)}`;
    const baseline = {};
    ...
    try {
        ...
        const idx = records.findIndex((r) => r.id === id);
        if (idx !== -1) records[idx] = body.category;
        render();
    } catch {
...
function renderListView() {
    titleEl.textContent = t(SCREEN_LABEL_KEY);
    ...
    newBtn.innerHTML = `<i class="bx bx-plus" aria-hidden="true"></i><span>${t('home.catNewButton')}</span>`;
    ...
    if (!records.length) {
        ...
        empty.textContent = t('home.catListEmpty');
    ...
    records.forEach((record) => {
        ...
        const offlineBadge = offlinePendingRecordIds.has(`${CATEGORY_TYPE}:${record.id}`)
        ...
        row.innerHTML = `
            <span class="home-carga-active-row-icon"><i class="bx bx-purchase-tag" aria-hidden="true"></i></span>
...
function renderFormView() {
    ...
    codeRow.textContent = `${t('main.colCatCode')}: ${record.code || '—'}`;
...
(async function init() {
    ...
    try {
        ...
        console.error('Nuestras Categorías (App) failed to load:', err);
