// ---------------------------------------------------------------------------
// Nuestras Cotizaciones (App) — same one-field-at-a-time shell as Tipos de
// Unidad (see AppTiposUnidad.js for the original of this pattern): a rate
// never "graduates out" the way a finished operational record does, so
// there's just one flat list (nothing ever disappears), tap any row to
// edit. Costo Total is never a field here -- it's server-derived (Costo por
// KM × Distancia Estimada, see mapFreightQuoteRecord in server.js) and just
// displayed once both of those are set.
// ---------------------------------------------------------------------------
...
// --- Field catalog -----------------------------------------------------
// unitTypeName's options come from the same catalog Nuestras Unidades
// already reads from -- fetched once at load (see loadUnitTypeOptions).
const FIELDS = [
    { id: 'routeName', apiKey: 'routeName', labelKey: 'main.colCotRuta', hintKey: 'home.freightQuoteHintRuta', type: 'text', icon: 'bx-map' },
    { id: 'unitTypeName', apiKey: 'unitTypeName', labelKey: 'main.colCotTipoUnidad', hintKey: 'home.freightQuoteHintTipoUnidad', type: 'select', icon: 'bx-car', optionGroups: [{ options: [] }] },
    { id: 'costPerKm', apiKey: 'costPerKm', labelKey: 'main.colCotCostoKm', hintKey: 'home.freightQuoteHintCostoKm', type: 'number', icon: 'bx-dollar' },
    { id: 'distanceKm', apiKey: 'distanceKm', labelKey: 'main.colCotDistanciaKm', hintKey: 'home.freightQuoteHintDistanciaKm', type: 'number', icon: 'bx-ruler' },
    { id: 'validityDate', apiKey: 'validityDate', labelKey: 'main.colCotVigencia', hintKey: 'home.freightQuoteHintVigencia', type: 'text', icon: 'bx-calendar' },
    { id: 'customerName', apiKey: 'customerName', labelKey: 'main.colCotCliente', hintKey: 'home.freightQuoteHintCliente', type: 'text', icon: 'bx-buildings' },
    {
        id: 'status', apiKey: 'status', labelKey: 'main.colCotEstatus', hintKey: 'home.freightQuoteHintEstatus', type: 'select', icon: 'bx-toggle-left',
...
async function loadUnitTypeOptions() {
    try {
        ...
        const field = FIELDS.find((f) => f.id === 'unitTypeName');
...
function blankDraftRecord() {
    return { id: null, folio: '', routeName: '', unitTypeName: '', costPerKm: '', distanceKm: '', totalCost: 0, validityDate: '', customerName: '', status: '', pendingFields: [] };
}
...
function recordLabel(record) {
    if (record.routeName) return record.routeName;
    if (!record.id) return t('home.freightQuoteNewButton');
    return `${t('home.freightQuoteFallbackLabel')} ${record.folio}`;
}

// --- Data loading ------------------------------------------------------
async function loadRecords() {
    try {
        const res = await fetch(apiUrl('/api/business/freight-quotes'), { credentials: 'include' });
        if (!res.ok) return;
        const { freightQuotes } = await res.json();
        records = freightQuotes || [];
    } catch {
        records = [];
    }
}
...
async function patchRecord(id, patch) {
    ...
    try {
        ...
        const idx = records.findIndex((r) => r.id === id);
        if (idx !== -1) records[idx] = body.freightQuote;
        render();
    } catch {
...
async function deleteRecord(id) {
    try {
        const res = await fetch(apiUrl(`/api/business/freight-quotes/${id}`), { method: 'DELETE', credentials: 'include' });
...
function renderListView() {
    titleEl.textContent = t('menu.opTransVolCotizaciones');
    ...
    newBtn.innerHTML = `<i class="bx bx-plus" aria-hidden="true"></i><span>${t('home.freightQuoteNewButton')}</span>`;
    newBtn.addEventListener('click', createNewFreightQuote);
    bodyEl.appendChild(newBtn);

    if (!records.length) {
        const empty = document.createElement('p');
        empty.className = 'home-carga-empty-note';
        empty.textContent = t('home.freightQuoteListEmpty');
    ...
    records.forEach((record) => {
        ...
        const offlineBadge = offlinePendingRecordIds.has(`nuestras-cotizaciones:${record.id}`)
        ...
        row.innerHTML = `
            <span class="home-carga-active-row-icon"><i class="bx bx-receipt" aria-hidden="true"></i></span>
...
function renderFormView() {
    ...
    codeRow.textContent = `${t('main.colCotFolio')}: ${record.folio || '—'}`;
    bodyEl.appendChild(codeRow);

    // Read-only, server-derived -- shown but never opens a field body.
    if (record.costPerKm && record.distanceKm) {
        const totalRow = document.createElement('p');
        totalRow.className = 'home-carga-progress-label';
        totalRow.hidden = false;
        totalRow.style.margin = '0 0 0.8rem';
        totalRow.style.fontWeight = '700';
        const amount = (record.totalCost || (record.costPerKm * record.distanceKm) || 0).toLocaleString('es-MX', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        totalRow.textContent = `${t('main.colCotCostoTotal')}: $${amount}`;
...
async function commitFieldValue(field, value) {
    ...
    if (draftRecord) {
        try {
            const res = await fetch(apiUrl('/api/business/freight-quotes'), { method: 'POST', credentials: 'include' });
            if (!res.ok) throw new Error('create failed');
            const { freightQuote } = await res.json();
            records.push(freightQuote);
            openRecordId = freightQuote.id;
            draftRecord = null;
            await patchRecord(freightQuote.id, { [field.apiKey]: value });
        } catch {
            showToast(t('admin.saveError'));
            draftRecord = null;
            view = 'list';
...
(async function init() {
    ...
    try {
        ...
        await Promise.all([loadRecords(), loadBrandingForTheme(), loadUnitTypeOptions()]);
        applyStyle(getStoredStyle());
        await refreshOfflinePendingIds();
    } catch (err) {
        console.error('Nuestras Cotizaciones (App) failed to load:', err);
