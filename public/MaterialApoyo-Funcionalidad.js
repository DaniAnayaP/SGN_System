// ---------------------------------------------------------------------------
// Funcionalidad Pantallas (Material Apoyo) — document library scoped to the
...
// ---------------------------------------------------------------------------
const CATEGORY = 'funcionalidad-pantallas';
...
(async function init() {
    try {
        await Dashboard.initDashboard({ activePage: 'cat-material-apoyo-funcionalidad' });
        refreshScope();
        await loadMaterials();
    } catch (err) {
        console.error('Material Apoyo (Funcionalidad Pantallas) failed to initialize:', err);
    }
})();
