// ---------------------------------------------------------------------------
// Nuestros Procesos (Material Apoyo) — document library scoped to the
...
// ---------------------------------------------------------------------------
const CATEGORY = 'nuestros-procesos';
...
// "Nuestros Procesos" + Apodo Empresa, built once clientBranding is
// available after initDashboard -- same fallback/format already used by
// Dashboard.js's own personalized-report labels (companyNickname falls
// back to companyName).
function applyCompanyNicknameTitle() {
    const nickname = Dashboard.companyNickname;
    if (!nickname) return;
    const label = `${Dashboard.t('menu.catMaterialApoyoProcesos')} — ${nickname}`;
    document.title = label;
    const welcomeText = document.querySelector('.welcome-text');
    if (welcomeText) welcomeText.textContent = label;
}

(async function init() {
    try {
        await Dashboard.initDashboard({ activePage: 'cat-material-apoyo-procesos' });
        applyCompanyNicknameTitle();
        refreshScope();
        await loadMaterials();
    } catch (err) {
        console.error('Material Apoyo (Nuestros Procesos) failed to initialize:', err);
    }
})();
