...
async function loadGateMap() {
    try {
        const res = await fetch(apiUrl('/api/business/field-fill-rules?tableKey=carga-combustible'), { credentials: 'include' });
        if (!res.ok) return;
...
async function loadRecords() {
    try {
        const res = await fetch(apiUrl('/api/business/fuel-loading-records'), { credentials: 'include' });
        if (!res.ok) return;
...
async function loadFleetFuelSuggestions() {
    try {
        const res = await fetch(apiUrl('/api/business/fleet-units'), { credentials: 'include' });
        if (!res.ok) return;
