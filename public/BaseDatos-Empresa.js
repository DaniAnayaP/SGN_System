// ---------------------------------------------------------------------------
// Base de Datos Global — read-only union of every record from every pantalla
// that has Control Interno columns wired (today: only Registro Combustible).
// No inline edit, no "+ Nuevo Registro", no delete — records are created and
// edited from their own pantalla; this screen is purely a consolidated view,
// with the Control Interno columns as the shared context that will let a
// future Reportes screen relate rows across pantallas. Column set today
// mirrors OpTransVolCombustible.html's own 41 columns exactly (13 Control
// Interno + Registro Combustible's own 27 + Acciones) since that's the only
// source table wired so far — a future pantalla's own columns get appended
// to the header the same way when it gets Control Interno too.
// ---------------------------------------------------------------------------
...
// One <tr> per Registro Combustible record — same column set/order as
// OpTransVolCombustible.js's own buildRow, minus every write affordance
// (attachInlineEdit, <select>s, evidence upload) since this view never
// patches anything back.
...
function getTbody() {
    return document.querySelector('[data-table-id="base-datos-global"] tbody');
}
...
async function loadRecords() {
    ...
    try {
        ...
        console.error('Base de Datos Global: failed to load records', err);
    }
}
...
(async function init() {
    try {
        ...
        console.error('Base de Datos Global failed to initialize:', err);
    }
})();
